...
const TiktokAffiliateView: React.FC<TiktokAffiliateViewProps> = ({ onReEdit, onCreateVideo, language, currentUser, onUserUpdate }) => {
    ...
    const generateOneImage = useCallback(async (index: number, attempt = 1, excludedServers: string[] = []) => {
        ...
        if (isPersonalOnly) {
            ...
            // We ignore 'selectedServer' in Hybrid mode to prevent bottlenecks.
            const availableServers = SERVERS.filter(s => !excludedServers.includes(s.url));
            
            if (availableServers.length === 0) {
               // Reset if all excluded (shouldn't happen easily with 10 servers and max 3 attempts, but safe fallback)
